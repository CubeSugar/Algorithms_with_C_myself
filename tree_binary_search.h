 ...
 *-----------------------------------------------------------------------------*/
void initBiSearchTree(BiSearchTree *biSearchTree, int (*compare)(const void *key1, const void *key2),
                      void (*destroy)(void *data));
/*-----------------------------------------------------------------------------
 ...
 *function:	在由参数tree所制定的二叉搜索树中移除数据同参数data相吻合的结点。实际上，
 			该操作仅仅只执行一种惰性移除，即该结点只是简单地被标记为隐藏。因此，并不
 			会返回指向结点数据的指针。更进一步，树中的数据就算被移除了也必须保持有效。
 			结果就是，二叉搜索树的大小，即biSearchTree.size值，并不会在移除一个结点
 			后递减。
 *-----------------------------------------------------------------------------*/
